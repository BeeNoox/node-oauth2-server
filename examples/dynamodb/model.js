...
model.getClient = function (clientId, clientSecret, callback) {
  ...
  dal.doGet(OAuthClientTable, { clientId: { S: clientId }}, true, function(err, data) {
    return callback(err, data && data.clientSecret === clientSecret && data);
  });
};
...
model.saveRefreshToken = function (refreshToken, clientId, expires, user, callback) {
  console.log('in saveRefreshToken (refreshToken: ' + refreshToken + ', clientId: ' + clientId +
    ', userId: ' + user.id + ', expires: ' + expires + ')');
...
model.getRefreshToken = function (bearerToken, callback) {
  ...
  dal.doGet(OAuthRefreshTokenTable, { refreshToken: { S: bearerToken }}, true, function(err, data) {
    if (data && data.expires) data.expires = new Date(data.expires * 1000);
    callback(err, data);
  });
};
...
model.saveAuthCode = function (authCode, clientId, expires, user, callback) {
  console.log('in saveAuthCode (authCode: ' + authCode + ', clientId: ' + clientId + ', userId: ' +
    user.id + ', expires: ' + expires + ')');
